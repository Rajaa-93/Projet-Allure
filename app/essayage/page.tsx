...
import { getCartProductIdForTryOnProduct } from "@/lib/cartProducts";
import { useCart } from "@/lib/useCart";
import { products, type Product, type ProductVariant } from "@/lib/tryOnProducts";
import {
...
export default function EssayagePage() {
  ...
  function addSelectedProductToCart() {
    if (!selectedProduct || !selectedSize) {
      setMessage("Choisis un article et une taille avant de l’ajouter au panier.");
      return;
    }

    addItem(
      getCartProductIdForTryOnProduct(selectedProduct.id),
      selectedSize,
      selectedVariantName || undefined
    );

    setMessage(
      `${selectedProduct.name} ${selectedVariantName ? `(${selectedVariantName}) ` : ""}a été ajouté au panier.`
    );
  }
      ...
      <div className="h-full overflow-y-auto px-6 pb-32 pt-14">
        ...
        <section className="relative z-10">
          <div className="text-center">
            <AllureLogo className="relative mx-auto h-28 w-64" priority />
          </div>
                ...
                {selectedProduct && (
                  <div className="mt-5 rounded-[24px] border border-[#e0d2bc] bg-[#f6f1e7] p-4">
                    ...
                    <button
                      ...
                      className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-[#1d1813] px-4 py-3 text-sm font-semibold text-[#f7e7b6] shadow-[0_12px_26px_rgba(29,24,19,0.22)]"
                    >
                      <ShoppingBag size={17} />
                      Ajouter au panier
                    </button>
